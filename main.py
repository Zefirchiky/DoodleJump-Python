...
doodle_right = pg.transform.scale(
    pg.image.load("img/doodle-right.png"), (PLAYER_W, PLAYER_H)
)
doodle_left = pg.transform.scale(
    pg.image.load("img/doodle-left.png"), (PLAYER_W, PLAYER_H)
)
platform_img = pg.transform.scale(
    pg.image.load("img/1jump_panel.png"), (PLATF_W, PLATF_H)
)
...
class Map:
    def __init__(self):
        self.map_move_y = 50
        self.platfsx = [
            randint(20, WIDTH - PLATF_W - 20) for _ in range(19)
        ]  # Firsts platforms X coords
        self.platfsy = [
            HEIGHT
            - 50
            - randint(
                i * round(PLATFS_H / (1 + 0.2 / (i + 1))),
                i * round(PLATFS_H * (1 + 0.2 / (i + 1))),
            )
            for i in range(19)
        ]  # Firsts platforms Y coords (with random range 0.4)
    ...
    def create(self):  # Add/Delete platforms
        if len(self.platfsx) <= 20:
            self.platfsx.append(
                randint(20, WIDTH - PLATF_W - 20)
            )  # Create platforms X coords
            self.platfsy.append(
                self.platfsy[-1] - randint(int(PLATFS_H // 1.2), int(PLATFS_H * 1.2))
            )  # Create platforms Y coords (with random range 0.4)
...
class Player(object):
    ...
    def collision(self, game_map, j):  # Collision player\platforms
        self.player_rect = pg.Rect(
            self.playerx, self.playery, PLAYER_W, PLAYER_H
        )  # Player rect
        for i in range(len(game_map.platfsx)):  # All platforms
            """Player touch platform   AND   player don`t jump   AND   player go down"""
            if (
                self.player_rect.colliderect(
                    pg.Rect(game_map.platfsx[i], game_map.platfsy[i], PLATF_W, PLATF_H)
                )
                and self.jump == False
                and self.y_change >= 0
            ):
    ...
    def death(self, keys):
        if self.playery > HEIGHT and keys[pg.K_f] == False:  # If player touch ground
            DIS.blit(
                death_font.render(TEXT_DEATH, True, RED),
                [10, HEIGHT / 2 - DEATH_FONT_SIZE / 2],
            )
            if self.DEATH:
                self.death_data_add()
                self.DEATH = False
    ...
    def score_read(self, score):
        with open("data.json", "w") as f:
            json.dump(self.data or {}, f, indent=4)
        ...
        if self.player_name in self.data:
            ...
            self.data[self.player_name] = {"Score": 0, "Death": self.data_death}
...
def main():
    ...
    while run:
        ...
        while not_rest:
            ...
            game_map.texts(
                score_font, f"Score: {game_map.score}", WHITE, BLACK, 50, 100
            )
